...
CONTAINS
   ...
   PURE REAL(dp) FUNCTION math__mean_1d(dd_array, dd_fill)
      ...
      IF( PRESENT(dd_fill) )THEN
         ...
         IF( il_count > 0 )THEN
            ...
            math__mean_1d=dl_sum/dl_count
         ELSE
            math__mean_1d=dd_fill
         ENDIF
      ELSE
   ...
   PURE REAL(dp) FUNCTION math__mean_2d(dd_array, dd_fill)
      ...
      IF( ALLOCATED(dl_list) )THEN
         math__mean_2d=math_mean(dl_list(:))
         DEALLOCATE( dl_list )
      ENDIF
   ...
   PURE REAL(dp) FUNCTION math__median_1d(dd_array, dd_fill)
      ...
      IF( ALLOCATED(dl_list) )THEN
         ...
         IF( MOD(il_count,2) == 0 )THEN
            math__median_1d=(dl_list(il_count/2+1)+dl_list(il_count/2))/2_dp
         ELSE
            math__median_1d=dl_list(il_count/2+1)
         ENDIF
   ...
   PURE REAL(dp) FUNCTION math__median_2d(dd_array, dd_fill)
      ...
      IF( ALLOCATED(dl_list) )THEN
         math__median_2d=math_median(dl_list(:))
         DEALLOCATE(dl_list)
      ENDIF
   ...
   PURE REAL(dp) FUNCTION math__mwe_1d(dd_array, id_next, dd_fill)
      ...
      IF( ALLOCATED(dl_list) )THEN
         ...
         IF( il_count == il_size )THEN
            ! no fillValue
            math__mwe_1d=math_mean(dl_list(il_next+1:il_size-il_next))
         ELSEIF( il_count > il_size-2*il_next )THEN
            ! remove one extremum each side
            math__mwe_1d=math_mean(dl_list(2:il_size-1))
         ELSE ! il_count <= il_size-2*il_next
            ! more than 2*il_next fillValue
            ! compute mean only
            math__mwe_1d=math_mean(dl_list(:))
         ENDIF
   ...
   PURE REAL(dp) FUNCTION math__mwe_2d(dd_array, id_next, dd_fill)
      ...
      IF( PRESENT(dd_fill)  )THEN
         ...
         IF( il_count > 0 )THEN
            ...
            math__mwe_2d=math_mwe(dl_list(:), id_next)
         ELSE
            math__mwe_2d=dd_fill
         ENDIF
      ELSE
         il_count=SIZE(dd_array(:,:))
         IF( il_count > 0 )THEN
   ...
   !-------------------------------------------------------------------
   REAL(dp) RECURSIVE FUNCTION math_compute(cd_var) RESULT(dd_res)
      IMPLICIT NONE
      ! Argument      
      ...
      ELSE
         ...
         ELSE
            ...
            IF( il_ind /= 0 )THEN
               ...
               dd_res=math_compute(cl_str1)*math_compute(cl_str2)
            ELSE
               il_ind=SCAN(TRIM(cl_var),'/')
               IF( il_ind /= 0 )THEN
                  cl_str1=cl_var(1:il_ind-1)
   ...
   PURE FUNCTION math_deriv_1D( dd_value, dd_fill, ld_discont )
      ...
      ! function
      REAL(dp), DIMENSION(SIZE(dd_value,DIM=1) ) :: math_deriv_1D
   ...
   FUNCTION math_deriv_2D( dd_value, dd_fill, cd_dim, ld_discont )
      ...
      &                   SIZE(dd_value,DIM=2) ) :: math_deriv_2D
   ...
   PURE FUNCTION math_deriv_3D( dd_value, dd_fill, cd_dim, ld_discont )
      ...
      REAL(dp), DIMENSION(SIZE(dd_value,DIM=1), &
      &                   SIZE(dd_value,DIM=2), &
      &                   SIZE(dd_value,DIM=3)) :: math_deriv_3D
