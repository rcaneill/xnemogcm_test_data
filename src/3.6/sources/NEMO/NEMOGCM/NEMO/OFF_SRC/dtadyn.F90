MODULE dtadyn
   ...
   LOGICAL            ::   ln_ssh_ini       !: initial ssh from dyn file (T) or not (F) - ssh is then read from passive tracer restart
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: dtadyn.F90 7523 2017-01-03 10:06:57Z cetlod $
...
CONTAINS
   ...
   SUBROUTINE dta_dyn( kt )
      ...
      !
      IF( lk_ldfslp .AND. .NOT.lk_c1d )   CALL  dta_dyn_slp( kt )    ! Computation of slopes
      !
      ...
      IF( ln_dynrnf ) THEN 
         ...
         IF( ln_dynrnf_depth .AND. lk_vvl )    CALL  dta_dyn_hrnf
      ENDIF
      !
      ...
      IF( lk_vvl ) THEN
         ...
         CALL dta_dyn_ssh( kt, zhdivtr, sshb, zemp, ssha, fse3t_a(:,:,:) )  !=  ssh, vertical scale factor & vertical transport
...
!         CALL prt_ctl(tab2d_1=qsr              , clinfo1=' qsr     - : ', mask1=tmask, ovlap=1 )
   ...
   SUBROUTINE dta_dyn_init
      ...
      NAMELIST/namdta_dyn/cn_dir, ln_dynrnf, ln_dynrnf_depth,  ln_ssh_ini, fwbcorr, &
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namdta_dyn in configuration namelist', lwp )
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '      ssh initialised from dyn file (T) or not (F)     ln_ssh_ini       = ', ln_ssh_ini
      ...
      !
      IF( lk_vvl ) THEN
        IF( ln_ssh_ini ) THEN                     ! Restart: read in restart file
           IF(lwp) WRITE(numout,*) ' sshn forcing fields read in the dynamics restart file for initialisation'
           CALL iom_open( 'restart', inum )
           CALL iom_get( inum, jpdom_autoglo, 'sshn', sshn(:,:)   )
           CALL iom_get( inum, jpdom_autoglo, 'sshb', sshb(:,:)   )
           CALL iom_close( inum )                                        ! close file
        ELSE
           IF(lwp) WRITE(numout,*) ' sshn forcing fields read in passive tracers restart file for initialisation'
