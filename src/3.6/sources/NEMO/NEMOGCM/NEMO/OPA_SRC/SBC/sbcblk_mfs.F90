...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: sbcblk_mfs.F90 9567 2018-05-10 14:26:43Z mathiot $
...
!CDIR COLLAPSE
   ...
   SUBROUTINE fluxes_mfs(alat,alon,hour,                               &
      ...
      !!----------------------------------------------------------------------
            ...
            esre  = shms(ji,jj) - shnow(ji,jj) * tmask(ji,jj,1)   ! --- calculates the term : qs(Ta)-qa(t2d)

            cseep = ce(ji,jj) * wndm(ji,jj) * esre * tmask(ji,jj,1)     ! --- calculates the term : Ce*|V|*[qs(Ts)-qa(t2d)]

            evap(ji,jj) = (cseep * rhom(ji,jj)) * tmask(ji,jj,1)  ! in [kg/m2/sec] !! --- calculates the EVAPORATION RATE [m/yr]

            elat(ji,jj) = rhom(ji,jj) * cseep * heatlat(sst(ji,jj)) * tmask(ji,jj,1)
   ...
   SUBROUTINE qshort(hour,alat,alon,cldnow,qsw)
      ...
      !!----------------------------------------------------------------------
          ...
          IF(cldnow(ji,jj).LT.0.3) THEN
             qsw(ji,jj) = qtot * (1.-albedo) * tmask(ji,jj,1)
          ELSE
             qsw(ji,jj) = qtot*(1.-0.62*cldnow(ji,jj)              &
                                + .0019*sunbet)*(1.-albedo) * tmask(ji,jj,1)
          ENDIF
