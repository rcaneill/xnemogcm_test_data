...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: sbcblk_clio.F90 9567 2018-05-10 14:26:43Z mathiot $ 
...
!CDIR NOVERRCHK
         DO ji = 1, jpi
            ...
            !
            zqlw(ji,jj) = - emic * stefan * ( ztaevbk + 4. * ztatm3 * ( zsst - ztatm ) ) * tmask(ji,jj,1) 
            ...
            ! sensible heat flux
            zqsb(ji,jj) = zrhova * zcsho * 1004.0  * ( zsst - ztatm ) * tmask(ji,jj,1) 
         
            ! latent heat flux (bounded by zero)
            zqla(ji,jj) = MAX(  0.e0, zrhova * zcleo * 2.5e+06 * ( zqsato - zqatm )  ) * tmask(ji,jj,1)
            !               
         END DO
      END DO
...
!CDIR NOVERRCHK
         DO ji = 1, jpi
            ...
            sprecip(ji,jj) = sf(jp_prec)%fnow(ji,jj,1) / rday * tmask(ji,jj,1)  &      ! rday = converte mm/day to kg/m2/s
...
!CDIR NOVERRCHK
            DO ji = 1, jpi
               ...
               !
               z_qlw(ji,jj,jl) = - emic * stefan * ( ztaevbk + 4. * ztatm3 * ( ptsu(ji,jj,jl) - ztatm(ji,jj) ) ) * tmask(ji,jj,1)
               ...
               !  sensible heat flux
               z_qsb(ji,jj,jl) = zrhovacshi * ( ptsu(ji,jj,jl) - ztatm(ji,jj) ) * tmask(ji,jj,1)
            
               !  latent heat flux 
               qla_ice(ji,jj,jl) = MAX(  0.e0, zrhovaclei * ( zqsati - zqatm(ji,jj) )  ) * tmask(ji,jj,1)
...
!CDIR COLLAPSE
      tprecip(:,:)   = sf(jp_prec)%fnow(:,:,1) / rday * tmask(:,:,1)        ! total precipitation [kg/m2/s]
      !
      ! ----------------------------------------------------------------------------- !
