...
!  G1%e2f(:,nyfin)=G1%e2f(:,nyfin-1)
  ...
  !
  WRITE(*,*) 'Position of the inner Child domain (i.e. without borders(masked) and ghostcells) : '
  IF( ln_agrif_domain ) THEN
     WRITE(*,*) 'latmin =',G1%gphit(2+nbghostcellsfine,2+nbghostcellsfine)
     WRITE(*,*) 'latmax =',G1%gphit(nxfin-(1+nbghostcellsfine),nyfin-(1+nbghostcellsfine))
     WRITE(*,*) 'lonmin =',G1%glamt(2+nbghostcellsfine,2+nbghostcellsfine)
     WRITE(*,*) 'lonmax =',G1%glamt(nxfin-(1+nbghostcellsfine),nyfin-(1+nbghostcellsfine))
  ELSE
