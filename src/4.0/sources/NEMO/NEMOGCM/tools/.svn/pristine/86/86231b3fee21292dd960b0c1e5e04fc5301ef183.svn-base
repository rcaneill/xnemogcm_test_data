...
CONTAINS
   ...
   SUBROUTINE iscpl_cons(ptmask_b, psmask_b, pe3t_b, pts_flx, pvol_flx, prdt_iscpl)
      ...
      REAL(wp):: r1_rdtiscpl
      REAL(wp):: zjip1_ratio  , zjim1_ratio  , zjjp1_ratio  , zjjm1_ratio
      !!
      REAL(wp):: zde3t, zdtem, zdsal
      REAL(wp), DIMENSION(:,:), POINTER :: zdssh
      !!
      ...
      CALL wrk_alloc(jpi,jpj, zdssh )

      ! get imbalance (volume heat and salt)
      ! initialisation difference
      zde3t = 0.0_wp; zdsal = 0.0_wp ; zdtem = 0.0_wp

      ! initialisation correction term
      pvol_flx(:,:,:  ) = 0.0_wp
      pts_flx (:,:,:,:) = 0.0_wp
      
      r1_rdtiscpl = 1._wp / prdt_iscpl 
         ...
         DO jj = 2,jpj-1
            DO ji = 2,jpim1
               IF (tmask_h(ji,jj) == 1._wp) THEN
                  ...
                  pvol_flx(ji,jj,jk)       =   pvol_flx(ji,jj,jk)        + zde3t * r1_rdtiscpl
                  pts_flx (ji,jj,jk,jp_sal)=   pts_flx (ji,jj,jk,jp_sal) + zdsal * r1_rdtiscpl 
                  pts_flx (ji,jj,jk,jp_tem)=   pts_flx (ji,jj,jk,jp_tem) + zdtem * r1_rdtiscpl
      ...
      ! deallocate variables
      CALL wrk_dealloc(jpi,jpj, zdssh ) 
