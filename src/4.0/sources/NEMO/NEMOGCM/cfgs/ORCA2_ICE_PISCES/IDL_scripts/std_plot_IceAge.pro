pro std_plot_IceAge, Iage1, Iage2, Ifra1, Ifra2, ARC = arc, ANT = ant, FEBR = febr, MARCH = march, SEPT = sept, POSTSCRIPT = postscript, _extra = ex
...
;
  ...
  Iage1.arr = Iage1.arr * ( Ifra1.arr gt 0.15 )
 
  plt, Iage1.arr - 1.E-04, MIN = vmin, MAX = vmax, INTER = vint, /STRICTFILL, CELL_FILL = 2, format = fmt $
...
;                                ;
  if std_file1_I NE std_file2_I then begin            
    ...
    Iage2.arr = Iage2.arr * ( Ifra2.arr gt 0.15 )
  
    plt, Iage1.arr - Iage2.arr, MIN = vmind, MAX = vmaxd, INTER = vintd, STYLE = 'so0so', format = fmtd $
