...
CONTAINS
  ...
  SUBROUTINE extrap_detect(G0,G1,detected,n,posvar) 
    ...
    ELSEIF( PRESENT(posvar) .AND. posvar == 'V' ) THEN
       CALL get_detected_pts(G0%gphiv,G1%gphiv,G0%glamv,G1%glamv,   &
            G0%vmask(:,:,n),G1%vmask(:,:,n),detected(:,:))                                  
    ELSEIF( PRESENT(posvar) .AND. posvar == 'F' ) THEN
       CALL get_detected_pts(G0%gphif,G1%gphif,G0%glamf,G1%glamf,   &
            G0%fmask(:,:,n),G1%fmask(:,:,n),detected(:,:))                                  
    ELSE
       CALL get_detected_pts(G0%gphit,G1%gphit,G0%glamt,G1%glamt,   &
            G0%tmask(:,:,n),G1%tmask(:,:,n),detected(:,:))                                  
!clem       CALL get_detected_pts(G0%nav_lat,G1%nav_lat,G0%nav_lon,G1%nav_lon,   &
!clem            G0%tmask(:,:,n),G1%tmask(:,:,n),detected(:,:))        
    ENDIF
    !      
  ...
  SUBROUTINE correct_field(detected_pts,tabin,tabinkm1,tabinkm2,G0,nav_lev,newmask,k,posvar)
    ...
    ELSE IF ( PRESENT(posvar) .AND. posvar == 'V' ) THEN
       ...
       lat = G0%gphiv 
       lon = G0%glamv
    ELSE IF ( PRESENT(posvar) .AND. posvar == 'F' ) THEN
       WHERE(G0%fmask(:,:,k) == 1. )
          newmask(:,:) = .TRUE.
       ELSEWHERE
          newmask(:,:) = .FALSE.
       END WHERE
       ALLOCATE(mask(SIZE(G0%fmask,1),SIZE(G0%fmask,2),SIZE(G0%fmask,3)))
       ALLOCATE(lat(SIZE(G0%fmask,1),SIZE(G0%fmask,2)))
       ALLOCATE(lon(SIZE(G0%fmask,1),SIZE(G0%fmask,2)))
       mask = G0%fmask        
       lat = G0%gphif 
       lon = G0%glamf
    ELSE      
