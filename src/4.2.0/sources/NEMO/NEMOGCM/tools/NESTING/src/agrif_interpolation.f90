...
CONTAINS
  ...
  SUBROUTINE agrif_interp(tabin,tabout,typevar)
    ...
    ELSE
       CALL agrif_base_interp3(tabin,tabout,imin,jmin,typevar)
    ENDIF
  ...
  SUBROUTINE agrif_base_interp2(tabin,tabout,i_min,j_min,typevar)
    ...
    IF (typevar .EQ. 'F') THEN
       ptx = 1 + nbghostcellsfine 
       pty = 1 + nbghostcellsfine 
       decalxc = 0.
       decalyc = 0.
       decalxf = 0.
       decalyf = 0.
    ELSEIF (typevar .EQ. 'T') THEN
       ptx = 2 + nbghostcellsfine 
       pty = 2 + nbghostcellsfine 
       decalxc = dxc/2.
       decalyc = dyc/2.
       decalxf = dxf/2.
       decalyf = dyf/2.
  ...
  !
  !      
  SUBROUTINE agrif_base_interp3(tabin,tabout,i_min,j_min,typevar)
    !
    IMPLICIT NONE
    REAL*8,DIMENSION(:,:) :: tabin,tabout
    INTEGER :: i_min,j_min
    ...
    CASE('F')
       IF(MOD(irafx,2)==1) THEN ! odd
          zx = 1 ; zy = 1 ; jdecx = irafx - 1 ; jdecy = irafy - 1
       ELSE                     ! even
          zx = 1 ; zy = 1 ; jdecx = irafx - 1 ; jdecy = irafy - 1
       ENDIF
    END SELECT
    ...
    !
  END SUBROUTINE agrif_base_interp3
  ...
  SUBROUTINE Interp_Extrap_var(filename, cd_type)      
    ...
    CHARACTER*100 :: interp_type
    CHARACTER*100 :: Child_file,Childcoordinates
    CHARACTER*100 :: varname,childbathy
    CHARACTER*1  :: posvar
    ...
    DO i = 1,SIZE(Ncdf_varname)
       ...
       CASE('time_steps')
          ...
          CALL Write_Ncdf_var('time_steps','time_counter',Child_file,tabtemp1DInt,'integer')
