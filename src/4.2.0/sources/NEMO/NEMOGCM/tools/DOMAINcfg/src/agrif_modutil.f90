...
CONTAINS
  ...
  !***********************************************************
  RECURSIVE SUBROUTINE quicksort(var, first, last)
     IMPLICIT NONE
     ...
     DO
        ...
        END DO
        IF (ji >= jj) EXIT
        t = var(ji);  var(ji) = var(jj);  var(jj) = t
        ji=ji+1
        jj=jj-1
     END DO
     IF (first < ji-1) CALL quicksort(var, first, ji-1)
     IF (jj+1 < last)  CALL quicksort(var, jj+1, last)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE make_bicubic_remap(tabin,masksrc,tabout,nxfin,nyfin,matrix,src_add,dst_add) 
    ...
    INTEGER :: in_x,is,ie,iw,ine,inw,ise,isw
    REAL*8 :: delew,delns
    !
    nx = SIZE(tabin,1)
    ny = SIZE(tabin,2)
    ...
    DO n = 1,nx*ny
       ...
       IF( grid1_mask(n) ) THEN                        
          ...
          ENDIF
          !
          gradj(n) = delns*(var1D(in_x) - var1D(is))                    
          !
          !*** compute ij-gradient
