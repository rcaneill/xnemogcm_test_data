pro std_plot_all_last_year_5D, doplot = doplot, _extra = ex
...
;; script for max MLD, output 5 days

  PRINT, ''
  PRINT, ' 	############################################'
  PRINT, ''
  PRINT, '                    LAUNCH of std_plots_last_year_5D'
  PRINT, ''
  PRINT, ' 	############################################'
  PRINT, ''
;
...
; emp (evaporation minus precipitation)
  ...
  ;mixed layer depth
  ;; nyear=jpt/73
  MLD_1 = read_ncdf(getenv('VAR1_MLD'), allrecords = allrec, filename = std_file1_T ) 
  caldat, time, mm
  march = where(mm EQ 3, cnt)
  MLD_march_1 = max(MLD_1.arr[*,*,march], dim=3)
  MLD_march_1 = {arr: MLD_march_1, unit: MLD_1.unit}
  sept = where(mm EQ 9, cnt)
  MLD_sept_1 = max(MLD_1.arr[*,*,sept], dim=3) 
  MLD_sept_1 = {arr: MLD_sept_1, unit: MLD_1.unit}
  MLD_1 = {arr: max(MLD_1.arr[*,*,*], dim=3), unit: MLD_1.unit}
  IF std_file2_T NE std_file1_T THEN BEGIN
    MLD_2 = read_ncdf(getenv('VAR2_MLD'), allrecords = allrec, filename = std_file2_T ) ; 10 m
    caldat, time, mm
    march = where(mm EQ 3, cnt)
    MLD_march_2 = max(MLD_2.arr[*,*,march], dim=3)
    MLD_march_2 = {arr: MLD_march_2, unit: MLD_2.unit}
    sept = where(mm EQ 9, cnt)
    MLD_sept_2 = max(MLD_2.arr[*,*,sept], dim=3) 
    MLD_sept_2 = {arr:MLD_sept_2, unit: MLD_2.unit}
    MLD_2 = {arr: max(MLD_2.arr[*,*,*], dim=3), unit: MLD_2.unit}
  ENDIF ELSE BEGIN
    MLD_2 = {arr:-1}
    MLD_march_2 = {arr:-1}
    MLD_sept_2 = {arr:-1}
...
; if monthly climatology
;  MLD = {arr: max(MLD.arr[*,*,*], dim=3), unit: MLD.unit} 
...
;
 ...
 ;
   ...
   Ifra_2 = read_ncdf(getenv('VAR2_Ifrac'), allrecords = allrec, filename = std_file2_I )
   msk = Ifra_2.arr gt 0.15 ; remove 0.15% for observations 
   IF std_file2_I NE std_file1_I THEN BEGIN
     Isal_2 = read_ncdf(getenv('VAR2_Isal'), allrecords = allrec, filename = std_file2_I ) 
     Isal_2.arr = Isal_2.arr * msk
     caldat, time, mm
     march = where(mm EQ 3, cnt)
     febr = where(mm EQ 2, cnt)
...
;
  ...
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_mld, MLD_1, MLD_2, MLD, _extra = ex

  ;SF: add 2 cases for max of Mixed Layer depth on march & septembre
  cnt = cnt+1   &   blabla = 'Mixed layer depth on March'
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_max_mld, MLD_march_1, MLD_march_2, /MARCH, _extra = ex

  cnt = cnt+1   &   blabla = 'Mixed layer depth on September'
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_max_mld, MLD_sept_1, MLD_sept_2, /SEPT, _extra = ex

;; SF: commented because not ok for max MLD, output 5 days
;; cnt = cnt+1   &   blabla = 'Zonal mean Mixed layer depth'
;; IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_ZonMld, MLD_1, MLD_2, MLD, _extra = ex
...
;
  ...
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_IceThick, Ithi_jan_1, Ithi_jan_2, /ARC, /JAN, _extra = ex
;
...
;
  ...
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_IceAge, Iage_march_1, Iage_march_2, /ARC, /MARCH, _extra = ex
;  
  cnt = cnt+1   &   blabla = 'Arctic Ice Age: SEPT'
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_IceAge, Iage_sept_1, Iage_sept_2, /ARC, /SEPT, _extra = ex
;  
  cnt = cnt+1   &   blabla = 'Antarctic Ice Age: MARCH'
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_IceAge, Iage_febr_1, Iage_febr_2, /ANT, /FEBR, _extra = ex
;
  cnt = cnt+1   &   blabla = 'Antarctic Ice Age: SEPT'
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_IceAge, Iage_sept_1, Iage_sept_2, /ANT, /SEPT, _extra = ex
;
...
;
  ...
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_SnowThick, Isnow_march_1, Isnow_march_2, /ARC, /MARCH, _extra = ex
; 
...
;  
  ...
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_SnowThick, Isnow_febr_1, Isnow_febr_2, /ANT, /FEBR, _extra = ex
; 
...
; 
  ...
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_IceSal, Isal_march_1, Isal_march_2, /ARC, /MARCH, _extra = ex
; 
  cnt = cnt+1   &   blabla = 'Arctic Ice Salinity: SEPT'
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_IceSal, Isal_sept_1, Isal_sept_2, /ARC, /SEPT, _extra = ex
;  
  cnt = cnt+1   &   blabla = 'Antarctic Ice Salinity: FEBRUARY'
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_IceSal, Isal_febr_1, Isal_febr_2, /ANT, /FEBR, _extra = ex
; 
  cnt = cnt+1   &   blabla = 'Antarctic Ice Salinity: SEPT'
  IF doplot EQ cnt OR doplot EQ 0 THEN std_plot_IceSal, Isal_sept_1, Isal_sept_2, /ANT, /SEPT, _extra = ex
; 
